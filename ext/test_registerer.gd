...
func _ready():
	...
	CommandServer.register_command(
		CommandBuilder.new().Literal("dmx").Literal("override").Literal("channel")
				.Branch().Validated("channel_index", is_valid_channel_index)
				.Branch().Literal("range").Validated("start_channel_index", is_valid_channel_index).Validated("end_channel_index", is_valid_channel_index)
			.EndBranch()
				.Branch().Validated("value", is_valid_dmx_value)
				.Branch().Literal("pattern").Variadic()
			.EndBranch()
		.Build()
	)
	CommandServer.register_command(
		CommandBuilder.new().Literal("dmx").Literal("override").Literal("universe")
				.Branch().Validated("universe_index", is_valid_universe_index)
				.Branch().Literal("range").Validated("start_universe_index", is_valid_universe_index).Validated("end_universe_index", is_valid_universe_index)
			.EndBranch()
				.Branch().Validated("value", is_valid_dmx_value)
				.Branch().Literal("pattern").Variadic()
			.EndBranch()
